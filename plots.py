...
#boxblots
data = [ naive[2],arima[2],sarima[2],autoarima[2],lstm[2],knner[2]]
...
ax.set_xticklabels(["Naive","ARIMA","SARIMA","AutoARIMA","LSTM","KNN"],fontsize=12)
...
x=["Naive","ARIMA","SARIMA", "AutoARIMA","LSTM","KNN"]
y=[naive[1],arima[1],23392,autoarima[1],lstm[1],knner[1]]
...
# Combine the data into a list
#boxblots
data = [ naive[2],np.average(arima1m[0],axis=1),np.average(sarima1m[0],axis=1),np.average(autoarima1m[0],axis=1), np.average(lstm1m[0],axis=1),np.average(knn1m[0],axis=1) ]
...
# Set the x-axis tick labels
ax2.set_xticklabels([ 'Naive',"ARIMA", 'SARIMA',"ΑutoARIMA","LSTM","KNN"],fontsize=12)
...
u=["Naive","ARIMA","SARIMA","AutoARIMA","LSTM","KNN"]
v=[np.sum(naive[1]),np.sum(arima1m[1]),np.sum(sarima1m[1]),np.sum(autoarima1m[1]),np.sum(lstm1m[1]),np.sum(knn1m[1])]
v=[abs(i) for i in v]
fig3, ax3 = plt.subplots(dpi=1000,figsize=(8, 7))
ax3.bar(u, v, color='grey', edgecolor='white', linewidth=0.5, alpha=1)
for i, value in enumerate(v):
    plt.text(i,value,str(int(value)),ha="center",va="bottom")
ax3.set_xticklabels(u,fontsize=12)
...
y_values = [7.61, 7.08, 7.83, 7.90, 8.82 ,9.04, 9.64, 9.15, 9.08, 8.52, 7.70, 7.62]

plt.figure(figsize=(12, 7),dpi=1000)
# Plotting
plt.plot(x_values, y_values,linewidth=4,color="grey")

# percentages as text labels
for i in range(len(x_values)):
    plt.annotate(f'{y_values[i]:.2f}%', (x_values[i], y_values[i]), textcoords="offset points", xytext=(0,10), ha='center',fontsize=13)

# Axis labels and title
plt.xlabel('Month',fontsize=15)
plt.ylabel('Water Consumption',fontsize=15)

# plt.title('Percentage Distribution')
plt.ylim(6.5, 10)
plt.yticks(np.arange(6.5,10,0.5 ))
